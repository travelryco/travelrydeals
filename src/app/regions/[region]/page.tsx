...
export default function RegionDealsPage({ params }: Props) {
  const { region } = params;
  ...
  // Load region-specific deals
  useEffect(() => {
    const fetchDeals = async () => {
      const regionDeals = await getDealsByRegion(regionName);
      setDeals(regionDeals);
      setLoading(false);
    };
  ...
  useEffect(() => {
    ...
    switch (sortOption) {
      case "price-low":
        sortedDeals.sort((a, b) => a.numeric_price - b.numeric_price);
        break;
      case "price-high":
        sortedDeals.sort((a, b) => b.numeric_price - a.numeric_price);
        break;
      case "discount":
      ...
      case "featured":
      default:
        sortedDeals.sort((a, b) => (a.featured === b.featured ? 0 : a.featured ? -1 : 1));
        break;
    }
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="animate-pulse">Loading {regionName} deals...</div>
      </div>
    );
  }
        ...
        <div className="max-w-7xl mx-auto py-16 px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center">
            ...
            <p className="max-w-2xl mx-auto text-xl text-white/90">
              Browse our exclusive member-only deals in {regionName}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
            <div>
              <h2 className="text-lg font-medium text-dark">
                Explore {regionName} Deals
              </h2>
              <p className="text-gray-600 mt-1">
                Find the best accommodations, activities and transportation in {regionName}
              </p>
            </div>
        ...
        <div className="mb-6">
          <p className="text-dark font-medium">
            Showing <span className="text-primary font-bold">{deals.length}</span> deals in {regionName}
          </p>
          <div className="flex mt-2">
            <Link href="/deals" className="text-secondary hover:text-secondary-dark font-medium flex items-center">
        ...
        {deals.length === 0 && (
          <div className="mt-8 text-center p-8 bg-white rounded-lg shadow-md border border-gray-100">
            ...
            <h3 className="text-lg font-medium text-dark mb-2">No deals found in {regionName}</h3>
            <p className="text-gray-500 mb-4">Check back later for new deals in this region.</p>
            <Link href="/deals" className="btn-secondary inline-flex items-center px-4 py-2">
              View all deals
            </Link>
          </div>
        )}
